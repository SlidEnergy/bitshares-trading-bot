...
var GenesisFilter = function () {
    ...
    GenesisFilter.prototype.isAvailable = function isAvailable() {
        return bts_genesiskeys_bloom_url !== undefined;
    };

    GenesisFilter.prototype.init = function init(done) {
        var _this = this;
    ...
    GenesisFilter.prototype.inGenesis = function inGenesis(pubkey_or_address) {
        if (!this.bloom_buffer) throw new Error("Call init() first");
        for (var hashes = 0; hashes < 3; hashes++) {
            var hex = _es.hash.sha256(hashes + ':' + pubkey_or_address);
    ...
    GenesisFilter.prototype.filter = function filter(account_keys, status) {
        var _this2 = this;
        ...
        this.init(function () {
            try {
                ...
                for (var a = 0; a < account_keys.length; a++) {
                    ...
                    status({ importing: true, account_name: keys.account_name, count: count, total: total });
                    for (var k = keys.encrypted_private_keys.length - 1; k >= 0; k--) {
                        ...
                        var addresses = _es.key.addresses(currentKey, 'BTS');
                    ...
                    status({ importing: false, account_name: keys.account_name,
                        count: count - removed_count, total: total });
